...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .hook{margin-top:18px; display:inline-block; padding:12px 16px; border-radius:999px; background:rgba(255,255,255,.10); border:1px solid rgba(255,255,255,.12); font-weight:700; color:#fff;}
    .layout{display:grid; grid-template-columns: 1.02fr .98fr; gap:22px; margin-top:24px;}
    .panel{background:var(--card); color:var(--ink); border-radius:var(--radius); box-shadow:var(--shadow); padding:22px; border:1px solid rgba(0,0,0,.04);}
    ...
    input[type=text], select, input[type=file]{width:100%; border:1px solid #d8deea; border-radius:14px; padding:13px 14px; font-size:15px; background:#fff; color:#0f172a;}
    input[type=file]{padding:11px 12px; background:#f8fbff}
    .hint{font-size:12px; color:#64748b; margin-top:4px;}
    .actions{display:flex; gap:10px; flex-wrap:wrap; margin-top:16px;}
    button, a.btn{appearance:none; border:none; border-radius:14px; padding:13px 16px; font-size:15px; font-weight:800; cursor:pointer; transition:transform .12s ease, opacity .12s ease, box-shadow .12s ease; text-decoration:none; display:inline-flex; align-items:center; justify-content:center}
    ...
    button.secondary, a.btn.secondary{background:#edf2ff; color:#312e81}
    ...
    .logs{margin-top:14px; background:#0f172a; color:#dbeafe; border-radius:16px; padding:14px; height:260px; overflow:auto; font:12px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace; white-space:pre-wrap;}
  ...
  <script>
    (function () {
      ...
      function setStatus(status, text) {
        statusPill.textContent = text || status || 'Queued';
      ...
      function renderResults(resultPages, runId) {
        ...
        for (const row of resultPages) {
          ...
          const name = esc(row.display_name || row.author_label || 'Author');
          const scopus = esc(row.scopus_id || '');
          ...
          html += `
            <div class="result-card">
              ...
              <div class="result-actions">
                <a class="btn secondary" href="${pageUrl}" target="_blank" rel="noopener">Open page</a>
                <button class="secondary share-btn" type="button" data-run-id="${esc(runId)}" data-author-label="${label}" data-note-id="${safeId}">Share to social</button>
              </div>
              <div class="share-note" id="${safeId}"></div>
            </div>`;
        }
        resultsEl.innerHTML = html;

        const shareButtons = resultsEl.querySelectorAll('.share-btn');
        shareButtons.forEach(function (btn) {
          btn.addEventListener('click', async function () {
            const runId = btn.getAttribute('data-run-id');
            const authorLabel = btn.getAttribute('data-author-label');
            const noteId = btn.getAttribute('data-note-id');
            const note = document.getElementById(noteId);
            btn.disabled = true;
            btn.textContent = 'Generating PNG...';
            try {
              const res = await fetch('/api/social-card/' + encodeURIComponent(runId) + '/' + encodeURIComponent(authorLabel), { method: 'POST' });
              const data = await res.json();
              if (!res.ok || !data.ok) throw new Error(data.error || 'Failed to generate social card.');
              note.classList.add('show');
              note.innerHTML = 'PNG download is available below for you to share on social media of your choice! <div style="margin-top:8px;"><a class="btn secondary" href="' + data.download_url + '">Download social PNG</a></div>';
      ...
      async function pollStatus() {
        ...
        if (['success', 'error', 'cancelled'].includes(data.status)) {
          ...
          killBtn.disabled = true;
          runBtn.disabled = false;
          if (data.status === 'error') currentStep.textContent = data.error || 'Run failed';
          if (data.status === 'cancelled') currentStep.textContent = 'Run stopped.';
        }
      }
