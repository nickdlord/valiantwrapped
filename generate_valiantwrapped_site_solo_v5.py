...
ORCID_COLS = ["ORCID", "Author ORCID", "Author(s) ORCID", "ORCIDs"]
ORCID_PATTERN = re.compile(r"\b\d{4}[-_ ]?\d{4}[-_ ]?\d{4}[-_ ]?\d{3}[\dXx]\b")
...
class AuthorRecord:
    ...
    professional_headshot_filename: str = ""
...
def normalize_orcid(value: object) -> str:
    ...
    if m:
        compact = re.sub(r"[^0-9Xx]", "", m.group(0)).upper()
...
def parse_author_identity_from_label(author_label: str) -> Dict[str, str]:
    parts = [p for p in canonical_author_label(author_label).split("_") if p]
    if not parts:
        return {"scopus_id": "", "orcid": "", "id_type": "", "name_parts": []}

    upper_parts = [p.upper() for p in parts]
    if "ORCID" in upper_parts:
        idx = max(i for i, part in enumerate(upper_parts) if part == "ORCID")
        orcid = normalize_orcid("_".join(parts[idx + 1:]))
        return {"scopus_id": "", "orcid": orcid, "id_type": "orcid" if orcid else "", "name_parts": parts[:idx]}

    scopus_id = normalize_scopus_id(parts[-1]) if parts else ""
    if scopus_id and len(parts) >= 2:
        return {"scopus_id": scopus_id, "orcid": "", "id_type": "scopus", "name_parts": parts[:-1]}

    return {"scopus_id": "", "orcid": "", "id_type": "", "name_parts": parts}


def normalized_author_key(value: object) -> str:
    identity = parse_author_identity_from_label(clean_text(value))
    parts = identity["name_parts"]
    if not parts:
        return ""
...
def infer_display_name(author_label: str) -> str:
    identity = parse_author_identity_from_label(author_label)
    name_parts = identity["name_parts"]
    if len(name_parts) >= 2:
        last = name_parts[0]
        first = " ".join(name_parts[1:])
        return f"{first.replace('_', ' ')} {last.replace('_', ' ')}".strip()
    if name_parts:
        return " ".join(part.replace("_", " ") for part in name_parts)
    return canonical_author_label(author_label).replace("_", " ")


def extract_scopus_id(author_label: str) -> str:
    return parse_author_identity_from_label(author_label)["scopus_id"]


def extract_orcid(author_label: str) -> str:
    return parse_author_identity_from_label(author_label)["orcid"]
...
def build_lifetime_metrics_map(author_csv_dir: Path) -> Dict[str, Metrics]:
    ...
    for csv_path in sorted(author_csv_dir.glob("*.csv")):
        ...
        citation_series = pd.to_numeric(df[cite_col], errors="coerce").fillna(0) if cite_col else pd.Series([0] * len(df.index))
        citation_count = int(citation_series.sum()) if len(citation_series) else 0

        top_journal = ""
        if journal_col and not df.empty:
            journal_stats = (
                df.groupby(journal_col, dropna=False)[cite_col] if cite_col else df.groupby(journal_col, dropna=False)
            )
            if cite_col:
                journal_stats = (
                    df.groupby(journal_col, dropna=False)[cite_col]
                    .agg(pub_count="size", cite_sum=lambda s: pd.to_numeric(s, errors="coerce").fillna(0).sum())
                    .reset_index()
                    .sort_values(["pub_count", "cite_sum"], ascending=[False, False])
                )
            else:
                journal_stats = (
                    df.groupby(journal_col, dropna=False)
                    .size()
            ...
            if not journal_stats.empty:
                top_journal = clean_text(journal_stats.iloc[0][journal_col])

        top_paper_title = ""
        top_paper_citations = 0
        if title_col and not df.empty:
            tmp = df.copy()
            tmp["__cites__"] = citation_series if len(citation_series) == len(tmp.index) else 0
            tmp = tmp.sort_values(["__cites__"], ascending=[False])
            top_paper_title = clean_text(tmp.iloc[0][title_col]) if title_col else ""
            top_paper_citations = int(tmp.iloc[0]["__cites__"]) if not tmp.empty else 0
...
def build_share_card(author: AuthorRecord, output_path: Path, cover_src_dir: Path, musician_headshot_src_dir: Path, project_title: str) -> None:
    ...
    draw.rounded_rectangle((24, 24, size - 24, size - 24),
                           radius=48, fill=(15, 15, 15))
    draw.ellipse((1080, -180, 1660, 390), fill=(18, 92, 52))
    draw.ellipse((860, -120, 1335, 285), fill=(36, 36, 36))
    draw.ellipse((-210, 930, 340, 1495), fill=(42, 18, 82))
    draw.ellipse((1060, 1080, 1530, 1590), fill=(16, 64, 34))
    draw.rounded_rectangle((54, 54, size - 54, size - 54),
    ...
    album = add_round_corners(open_and_fill_square(album_path, 440), 34)

    font_brand = load_font(36, bold=True)
    font_eyebrow = load_font(26, bold=True)
    font_small = load_font(22, bold=False)
    font_stat_label = load_font(18, bold=False)
    font_section = load_font(25, bold=True)

    draw.text((84, 72), project_title, font=font_brand, fill=(30, 215, 96))
    draw.text((84, 110), 'Research, remixed.',
    ...
    draw.text((size - 84 - (share_bbox[2] - share_bbox[0]), 76),
    ...
    # Alter ego panel closer to edges
    artist_box_top = max(278, title_end_y + 20)
    artist_box_bottom = 740
    artist_box = (66, artist_box_top, 1334, artist_box_bottom)
    draw.rounded_rectangle(artist_box, radius=34, fill=(17, 17, 17))
    draw.text((94, artist_box_top + 24), 'YOUR MUSICAL ALTER EGO',
    ...
    artist_name_y = artist_box_top + 74
    ...
    draw.text((text_left, stats_heading_y), 'Lifetime Scopus Stats',
              font=load_font(18, bold=True), fill=(30, 215, 96))

    stat_y = artist_box_bottom - 72
    stats = [
        ('Papers', str(author.lifetime_pub_count)
         if author.lifetime_pub_count > 0 else (str(author.metrics.pub_count) if author.metrics.pub_count > 0 else '—')),
        ('Citations', str(author.lifetime_citation_count)
         if author.lifetime_citation_count > 0 else (str(author.metrics.citation_count) if author.metrics.citation_count > 0 else '—')),
    ]
    stat_x = text_left
    stat_gap = 152
    ...
    draw.text((406, stat_y + 5), 'Top journal',
              font=font_stat_label, fill=(173, 173, 173))
    journal_font = fit_font_for_multiline(
        draw, journal, 600, 20, min_size=15, bold=False, max_lines=2)
    draw_multiline_block(draw, journal, journal_font, (238, 238, 238),
                         406, stat_y + 25, 600, line_gap=3, max_lines=2)

    # Album + tracklist band, closer to edges
    lower_box = (54, 762, 1346, 1218)
    draw.rounded_rectangle(lower_box, radius=38, fill=(15, 15, 15))
    left_panel = (74, 818, 690, 1190)
    right_panel = (720, 818, 1288, 1118)
    ...
    draw.text((78, heading_y), 'Latest Album:',
    ...
    album_title_font = fit_font_for_multiline(
        draw, album_title, 540, 36, min_size=20, bold=True, max_lines=2)
    draw_multiline_block(draw, album_title, album_title_font, (242, 242, 242),
                         78 + label_width + 16, heading_y - 4, 540, line_gap=2, max_lines=2)
    draw.text((724, heading_y), 'TRACKLIST',
              font=font_section, fill=(30, 215, 96))
    ...
    for i, track in enumerate(tracks, start=1):
        ...
        draw.rounded_rectangle(
            (738, track_y, 1268, track_y + row_h - 6), radius=18, fill=(30, 30, 30))
        track_font = fit_font_for_multiline(
            draw, line, 488, 19, min_size=14, bold=False, max_lines=1)
        draw_multiline_block(draw, line, track_font, (235, 235, 235),
                             758, track_y + 6, 488, line_gap=2, max_lines=1)
        track_y += row_h

    # Encore text placed beneath the tracklist and clear of both the track rows and album art
    ...
    encore_font = fit_font_for_multiline(
        draw, encore_text, encore_max_width, 23, min_size=18, bold=True, max_lines=3)
    encore_lines = wrap_text_for_draw(
        draw, encore_text, encore_font, encore_max_width)
    encore_line_bbox = draw.textbbox((0, 0), 'Ag', font=encore_font)
    encore_line_h = encore_line_bbox[3] - encore_line_bbox[1]
    encore_text_h = len(encore_lines) * encore_line_h + \
        max(0, len(encore_lines) - 1) * 3
    encore_x = 758
    encore_y = max(track_y + 18, 1138)
    draw_multiline_block(draw, encore_text, encore_font, (245, 245, 245),
                         encore_x, encore_y, encore_max_width, line_gap=3, max_lines=3)
...
def professional_headshot_markup(author: AuthorRecord, depth_prefix: str = "") -> str:
    if author.professional_headshot_filename:
        src = f"{depth_prefix}assets/professional_headshots/{author.professional_headshot_filename}"
        alt = f"Professional headshot for {author.display_name}"
        return f'<img class="pro-headshot" src="{html_escape(src)}" alt="{html_escape(alt)}">'
    return '<div class="empty-state">Professional headshot not found.</div>'
...
def home_card_markup(author: AuthorRecord) -> str:
    ...
    search_blob = f"{real_name} {artist_name} {album}"
    return (
        f'<a class="author-card" href="authors/{slug}/" data-search="{html_escape(search_blob)}">'
        f'{art}'
...
def author_page_markup(author: AuthorRecord, project_title: str, tagline: str, closer: str) -> str:
    ...
    total_papers = author.lifetime_pub_count or author.metrics.pub_count
    total_citations = author.lifetime_citation_count or author.metrics.citation_count
    stat_items = [
        metric_chip("Publications", str(total_papers if total_papers > 0 else "—")),
        metric_chip("Citations", str(total_citations if total_citations > 0 else "—")),
    ]
    if clean_text(author.metrics.top_journal):
        stat_items.append(metric_chip("Top journal", author.metrics.top_journal))

    expertise_html = (
        f'<div class="summary-block solo-summary-block"><div class="metric-label">Author expertise</div><div class="top-paper-title">{html_escape(author.expertise_summary)}</div></div>'
        if clean_text(author.expertise_summary)
        else '<div class="summary-block solo-summary-block"><div class="metric-label">Author expertise</div><div class="top-paper-title">No expertise summary was available for this profile.</div></div>'
    )
  ...
  <div class="app-shell">
    <aside class="sidebar">
      ...
      <nav class="sidebar-nav">
        <a href="#persona" class="nav-link active">Artist persona</a>
...
.hero-top-row { display: flex; justify-content: space-between; align-items: flex-start; gap: 28px; }\n.hero-top-copy { flex: 1 1 auto; min-width: 0; }\n.hero-headshot-top { flex: 0 0 auto; display: flex; align-items: flex-start; justify-content: flex-end; }\n.hero-headshot-top .pro-headshot { width: 128px; height: 128px; object-fit: cover; border-radius: 50%; border: 4px solid rgba(29,185,84,.6); box-shadow: 0 12px 28px rgba(0,0,0,.42), 0 0 0 1px rgba(255,255,255,.05); display: block; background: linear-gradient(135deg, #161616, #0c0c0c); }\n.hero-headshot-top .empty-state { width: 128px; min-height: 128px; display: grid; place-items: center; text-align: center; border-radius: 22px; padding: 14px; }\n.hero-stats-title { font-size: 1.25rem; font-weight: 800; letter-spacing: -.02em; margin-bottom: 10px; color: #ffffff; }\n.hero-stats-header { display: flex; justify-content: space-between; align-items: flex-start; gap: 16px; }
...
.summary-block p, .summary-block .top-paper-title { font-size: 1.08rem; line-height: 1.65; }
.solo-metrics-grid { margin-top: 14px; }
.solo-summary-block { max-width: 980px; }
...
def build_site(
    ...
    professional_headshot_src_dir: Path,
    project_title: str,
    tagline: str,
    closer: str,
) -> None:
    ...
    professional_headshot_out_dir = assets_dir / "professional_headshots"
    ...
    ensure_dir(professional_headshot_out_dir)
    ...
    if musician_headshot_src_dir.exists():
        for portrait_path in musician_headshot_src_dir.glob("*.*"):
            if portrait_path.suffix.lower() in {".png", ".jpg", ".jpeg", ".webp"}:
                shutil.copy2(portrait_path, musician_headshot_out_dir / portrait_path.name)

    if professional_headshot_src_dir.exists():
        for headshot_path in professional_headshot_src_dir.glob("*.*"):
            if headshot_path.suffix.lower() in {".png", ".jpg", ".jpeg", ".webp"}:
                shutil.copy2(headshot_path, professional_headshot_out_dir / headshot_path.name)
            ...
            [
                {
                    ...
                    "artist_name": a.persona.artist_name,
                    "album_title": a.persona.album_title,
                    "share_card_filename": f"{slugify(a.author_label)}.png",
                }
                for a in authors
            ],
            ensure_ascii=False,
    ...
    if not authors:
        raise RuntimeError("No authors were provided to build_site().")

    primary_author = authors[0]
    primary_author.share_card_filename = f"{slugify(primary_author.author_label)}.png"
    build_share_card(
        author=primary_author,
        output_path=share_card_out_dir / primary_author.share_card_filename,
    ...
    (output_dir / "index.html").write_text(
        author_page_markup(primary_author, project_title, tagline, closer),
        encoding="utf-8",
    )
...
def parse_orcids(raw: str) -> List[str]:
    raw = clean_text(raw)
    if not raw:
        return []
    found = [normalize_orcid(m.group(0)) for m in ORCID_PATTERN.finditer(raw)]
    seen = set()
    ordered = []
    for item in found:
...
def build_paper_records(scopus_df: pd.DataFrame) -> List[PaperRecord]:
    ...
    orcid_col = pick_existing_col(scopus_df.columns, ORCID_COLS)
    ...
    for _, row in scopus_df.iterrows():
        ...
        records.append(
            PaperRecord(
                ...
                author_orcids_raw=author_orcids_raw,
                doi=doi,
...
def author_in_record(scopus_id: str, orcid: str, record: PaperRecord) -> bool:
    if scopus_id:
        record_ids = parse_author_ids(record.author_ids_raw)
        if scopus_id in record_ids:
            return True
        raw = clean_text(record.author_ids_raw)
        if re.search(rf"(?<!\d){re.escape(scopus_id)}(?!\d)", raw) is not None:
            return True

    if orcid:
        record_orcids = parse_orcids(record.author_orcids_raw)
        if normalize_orcid(orcid) in record_orcids:
            return True
        raw_orcid = clean_text(record.author_orcids_raw)
        normalized_raw = normalize_orcid(raw_orcid)
        if normalized_raw and normalized_raw == normalize_orcid(orcid):
            return True
...
def recommend_for_author(
    ...
    for idx in ranked_idx:
        ...
        if key in seen_titles:
            continue
        if author_in_record(author.scopus_id, author.orcid, record):
            continue
...
def collect_authors(
    ...
    professional_headshots_dir: Path,
    ...
    for label in labels:
        ...
        authors.append(
            AuthorRecord(
                ...
                orcid=extract_orcid(label),
                expertise_summary=expertise_map.get(
                    label) or expertise_map.get(normalized_label, ""),
                metrics=effective_metrics,
                lifetime_pub_count=lifetime_metrics.pub_count,
                lifetime_citation_count=lifetime_metrics.citation_count,
                persona=persona,
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Build the VALIANT Wrapped static website (solo-page variant).")
    ...
    ap.add_argument("--professional-headshots-dir",
                    default="author_headshots/documents",
                    help="Directory of real uploaded/professional headshots used in the hero section.")
...
def main() -> None:
    ...
    professional_headshots_dir = Path(args.professional_headshots_dir)
    ...
    authors = collect_authors(
        ...
        professional_headshots_dir=professional_headshots_dir,
    ...
    if len(authors) > 1:
        print(f"[solo] warning: {len(authors)} authors were found; building the solo page for the first assembled author only: {authors[0].author_label}")
    ...
    build_site(
        ...
        professional_headshot_src_dir=professional_headshots_dir,
