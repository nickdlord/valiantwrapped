...
- Defaults are intentionally aggressive so outputs are more transformed and only loosely resemble the source headshot.
...
NEGATIVE_PROMPT = (
    ...
    "crossed eyes, asymmetrical eyes, overexposed skin, deformed mouth, warped teeth, flat lighting, dull styling, "
    "boring corporate headshot, office portrait, lab coat, hospital coat, plain business attire, neutral background, "
...
def build_visual_prompt(persona: Persona, author_label: str) -> Tuple[str, str, Dict[str, str]]:
    ...
    lighting = first_matching_rule(blob, LIGHTING_RULES, "cinematic editorial stage lighting with strong contrast")
    wardrobe = first_matching_rule(blob, WARDROBE_RULES, "fashion-forward performance wardrobe with statement styling")
    background = first_matching_rule(blob, BACKGROUND_RULES, "dramatic stylized studio set with concert atmosphere")

    mood_words = []
    for token in [
        "confident", "charismatic", "cinematic", "editorial", "immersive",
        "moody", "bold", "rebellious", "glamorous", "electric", "intense"
    ]:
        if token in blob.lower() and token not in mood_words:
            mood_words.append(token)
    if not mood_words:
        mood_words = ["bold", "cinematic", "charismatic", "electric"]

    prompt = (
        "dramatically reimagined musician portrait, transformed into a "
        f"{genre}, {wardrobe}, {lighting}, {background}, "
        f"{', '.join(mood_words)} energy, striking editorial fashion, "
        "album-shoot aesthetic, dramatic hair and styling, bold accessories, "
        "stage-ready presence, cinematic color grading, highly stylized, photorealistic, "
        "visibly transformed from an academic headshot into a fictional music artist"
    )
