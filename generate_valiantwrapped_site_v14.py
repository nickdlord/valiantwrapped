...
def build_share_card(author: AuthorRecord, output_path: Path, cover_src_dir: Path, musician_headshot_src_dir: Path, project_title: str) -> None:
    ...
    draw.ellipse((760, -120, 1280, 400), fill=(20, 70, 40))
    draw.ellipse((-140, 760, 320, 1220), fill=(26, 26, 26))
    ...
    album = add_round_corners(open_and_fill_square(album_path, 420), 32)
    portrait = add_round_corners(open_and_fill_square(portrait_path, 250), 32)
    card.paste(album, (72, 86), album)
    card.paste(portrait, (262, 350), portrait)

    # Text
    font_brand = load_font(34, bold=True)
    font_name = load_font(72, bold=True)
    font_artist = load_font(56, bold=True)
    font_album = load_font(40, bold=False)
    font_small = load_font(28, bold=False)

    text_x = 560
    y = 96
    draw.text((text_x, y), project_title, font=font_brand, fill=(30, 215, 96))
    y += 74
    y = draw_multiline_block(draw, author.display_name, font_name, (245, 245, 245), text_x, y, 560, line_gap=10, max_lines=3)
    y += 18
    draw.text((text_x, y), 'Musical alter ego', font=font_small, fill=(179, 179, 179))
    y += 42
    y = draw_multiline_block(draw, author.persona.artist_name or 'Still waiting on the stage name reveal', font_artist, (255, 255, 255), text_x, y, 560, line_gap=8, max_lines=3)
    y += 18
    draw.text((text_x, y), 'Latest album', font=font_small, fill=(179, 179, 179))
    y += 42
    y = draw_multiline_block(draw, author.persona.album_title or 'Untitled drop', font_album, (220, 220, 220), text_x, y, 560, line_gap=8, max_lines=3)

    bottom_y = 860
    stat_line = f"{author.lifetime_pub_count or author.metrics.pub_count} total papers • {author.lifetime_citation_count or author.metrics.citation_count} total citations"
    draw.rounded_rectangle((72, bottom_y - 24, size - 72, size - 86), radius=28, fill=(24, 24, 24), outline=(48, 48, 48), width=2)
    draw.text((108, bottom_y + 20), 'Research, remixed.', font=font_artist, fill=(245, 245, 245))
    draw.text((108, bottom_y + 98), stat_line, font=font_small, fill=(179, 179, 179))
