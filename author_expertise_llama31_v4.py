...
def make_map_messages(author_id: str, chunk_text: str) -> List[Dict[str, str]]:
    sys = (
        ...
        "- When possible, describe the research themes directly rather than repeatedly referring to the person.\n\n"
    ...
    user = (
        ...
        "Focus on the work itself rather than repeatedly referring to the person."
    )
...
def make_reduce_messages(author_id: str, themes: List[str], evidence_notes: str) -> List[Dict[str, str]]:
    ...
    sys = (
        "You write concise, polished, website-ready research bios.\n"
        "Hard rules:\n"
        "- Use ONLY the evidence provided.\n"
        "- Do NOT invent facts.\n"
        "- Do NOT list paper titles, DOIs, or citations.\n"
        "- Write 1–2 paragraphs, 120–220 words total.\n"
        "- Include primary areas of expertise and, if supported by the evidence, how the research focus has evolved over time.\n"
        "- Use a natural, fluid academic tone suitable for a professional profile.\n"
        "- Do NOT assume, infer, or mention the author's gender.\n"
        "- Do NOT use gendered pronouns or gendered nouns such as he, she, his, her, himself, herself, chairman, or spokeswoman.\n"
        "- Avoid repetitive phrases such as 'the author,' 'the researcher,' or repeating the same subject in every sentence.\n"
        "- Prefer sentence structures that foreground the work itself, research areas, methods, contributions, or applications.\n"
        "- You may mention the author's full name once at the beginning if helpful, then minimize direct references.\n"
        "- Avoid personal descriptors and avoid repeating the author's name unnecessarily.\n"
        "- The final bio should read smoothly and naturally, not like it is trying to avoid pronouns.\n"
        "- Bad style example: 'The author studies X. The researcher also examines Y. The author further investigates Z.'\n"
        "- Good style example: 'Research in this area focuses on X, with additional emphasis on Y and Z.'\n"
    )
    user = (
        f"Author ID: {author_id}\n\n"
        ...
        "Keep the wording natural and professional, and avoid repetitive subject labels."
    )
    return [{"role": "system", "content": sys}, {"role": "user", "content": user}]
