...
BANNED_ARTIST_NAME_TOKENS = {
    "echo", "flux", "mind", "signal", "pulse", "spectrum", "neural",
    "data", "algorithm", "quantum", "wave", "frequency", "circuit",
    "static", "code", "logic", "byte", "matrix", "nexus", "core",
    "nova", "vortex", "shadow", "dream", "sonic"
}

BANNED_ARTIST_NAME_PHRASES = {
    "echo mind", "echo flux", "signal bloom", "data pulse", "quantum echo",
    "neural tide", "static bloom", "signal drift", "pulse theory"
}

NAME_ARCHETYPES = [
    "two-word poetic alias",
    "stylized stage name",
    "cinematic band-style name",
    "literary or mythic-feeling alias",
    "unexpected phrase that sounds like an indie, jazz, ambient, or art-pop act",
    "fashion-forward or avant-garde persona name",
]
...
def tokenize_name_parts(text: str) -> List[str]:
    norm = normalize_for_similarity(text)
    return [tok for tok in norm.split() if len(tok) >= 4]


def artist_name_style_flags(name: str) -> List[str]:
    flags: List[str] = []
    norm = normalize_for_similarity(name)
    parts = norm.split()

    if not norm:
        flags.append("artist name was empty")
        return flags

    if any(tok in BANNED_ARTIST_NAME_TOKENS for tok in parts):
        bad = sorted({tok for tok in parts if tok in BANNED_ARTIST_NAME_TOKENS})
        flags.append(f"artist name used banned generic token(s): {', '.join(bad)}")

    if any(phrase in norm for phrase in BANNED_ARTIST_NAME_PHRASES):
        flags.append("artist name matched a banned generic phrase pattern")

    if len(parts) == 1:
        flags.append("artist name was a single-word alias, which tends to become repetitive")

    if len(parts) >= 2 and all(part.isalpha() for part in parts):
        if parts[0] == parts[-1]:
            flags.append("artist name repeated the same word fragment")

    if len(parts) == 2:
        if parts[0].endswith(("al", "ic", "um")) or parts[1] in {"mind", "flux", "pulse", "signal", "wave"}:
            flags.append("artist name fit an overly templated tech-word pattern")

    return flags


def shared_name_fragment_count(name_a: str, name_b: str) -> int:
    a = set(tokenize_name_parts(name_a))
    b = set(tokenize_name_parts(name_b))
    return len(a & b)
...
def prior_examples_text(priors: List[Dict[str, object]], max_examples: int = 12) -> str:
    if not priors:
        return "None yet."
...
def recent_artist_fragments_text(priors: List[Dict[str, object]], max_names: int = 20) -> str:
    fragments: List[str] = []
    for p in priors[-max_names:]:
        fragments.extend(tokenize_name_parts(clean_text(p.get("artist_name", ""))))
    ...
    return ", ".join(unique[:30]) if unique else "None yet."
...
def make_persona_messages(
    ...
    archetype = NAME_ARCHETYPES[(max(1, attempt_num) - 1) % len(NAME_ARCHETYPES)]
    sys_msg = (
        ...
        "- The artist name MUST feel highly distinctive, memorable, and non-template.\n"
        "- Do NOT use generic sci-fi, tech, or abstract-word names.\n"
        "- Do NOT use or remix words like Echo, Flux, Mind, Signal, Pulse, Spectrum, Neural, Data, Quantum, Static, Wave, Vortex, Nexus, or close variants.\n"
        "- Avoid names built as <tech word> + <abstract noun> or one-word futuristic compounds.\n"
        "- Prefer names that sound like a plausible artist brand: poetic, cinematic, stylish, literary, mythic, or genre-specific.\n"
        "- Favor two or three word artist names, or a stylized stage name, rather than a generic single compound word.\n"
        "- Avoid vague stock phrases like 'turning data into melody', 'blending science and sound', or generic futuristic music cliches.\n"
        "- Avoid repeating names, album concepts, track-title phrasing, and imagery used for other authors.\n"
    )
    ...
    if retry_reason:
        retry_block = (
            ...
            f"- Move farther away from polished sci-fi placeholder naming.\n"
        )
    ...
    user_msg = (
        ...
        f"THEMES:\n{themes}\n\n"
        f"SUMMARY:\n{summary}\n\n"
        f"For this attempt, make the artist name style a: {archetype}.\n\n"
        "Previously accepted personas from other authors. Do NOT reuse, remix, or slightly modify these naming patterns. Avoid anything stylistically similar:\n"
        f"{prior_examples_text(prior_personas)}\n\n"
        "Recent artist-name fragments that are already overused. Do NOT reuse these fragments unless absolutely necessary:\n"
        f"{recent_artist_fragments_text(prior_personas)}\n"
        f"{retry_block}\n"
...
def find_similarity_issue(
    ...
    style_flags = artist_name_style_flags(cand_artist)
    if style_flags:
        return True, style_flags[0]
    ...
    for prev in prior_personas:
        ...
        if similarity(cand_artist, prev_artist) >= 0.75:
            return True, f"the artist name was too similar to an earlier one ({prev_artist})"

        if shared_name_fragment_count(cand_artist, prev_artist) >= 1:
            return True, f"the artist name reused a key word fragment from an earlier one ({prev_artist})"

        if similarity(cand_album, prev_album) >= 0.84:
...
def make_fallback_messages(
    ...
    sys_msg = (
        "You are a careful creative writer.\n"
        "Create a fictional musical persona from the source material below.\n\n"
        "Hard rules:\n"
        "- Output plain text only.\n"
        "- Do NOT output JSON.\n"
        "- Do NOT output markdown code fences.\n"
        "- Keep the tone vivid but grounded.\n"
        "- The bio should be one short paragraph, around 90-170 words.\n"
        "- Include exactly 10 track titles.\n"
        "- Use a distinctive artist name, but do NOT use generic tech-cliche words like Echo, Flux, Signal, Pulse, Neural, Quantum, Nexus, Vortex, Static, Spectrum, or Mind.\n"
        "- Favor human-feeling names, cinematic phrases, literary references, place-like names, or fashion-forward aliases.\n"
        "- Avoid copying or closely echoing previously used artist names.\n"
        "- Do not mention real institutions, grants, papers, citation counts, or journals.\n"
    )
    ...
    if retry_reason:
        guidance = (
            "\nEXTRA GUIDANCE:\n"
            f"- Earlier attempts failed because: {retry_reason}\n"
            "- Move away from stock sci-fi/tech naming.\n"
            "- Make the artist name feel more like an actual performer or band.\n"
            "- Keep track titles concrete and varied rather than abstract placeholders.\n"
        )
    ...
    user_msg = (
        ...
        f"THEMES:\n{themes}\n\n"
        f"SUMMARY:\n{summary}\n\n"
        "Previously accepted personas. Do not repeat or closely echo these:\n"
        f"{prior_examples_text(prior_personas)}\n\n"
        "Overused artist-name fragments to avoid when possible:\n"
        f"{recent_artist_fragments_text(prior_personas)}\n"
        f"{guidance}\n"
        "Return EXACTLY in this format:\n\n"
        "Artist: <artist name>\n"
...
def find_similarity_issue_relaxed(
    candidate: Dict[str, object],
    prior_personas: List[Dict[str, object]],
) -> Tuple[bool, str]:
    cand_artist = clean_text(candidate.get("artist_name", ""))
    ...
    for prev in prior_personas:
        ...
        if similarity(cand_artist, prev_artist) >= 0.90:
            return True, f"the artist name was too similar to an earlier one ({prev_artist})"

        if shared_name_fragment_count(cand_artist, prev_artist) >= 2:
            return True, f"the artist name reused too many fragments from an earlier one ({prev_artist})"

        if similarity(cand_album, prev_album) >= 0.92:
            return True, f"the album title was too similar to an earlier one ({prev_album})"

        if similarity(cand_bio, prev_bio) >= 0.90:
            return True, f"the bio was too similar to an earlier one ({prev_artist} / {prev_album})"

    return False, ""


def normalize_persona_for_fallback(obj: Dict[str, object], explicit_gender_supported: bool) -> Dict[str, object]:
    normalized = dict(obj)
    normalized["artist_name"] = clean_text(normalized.get("artist_name", ""))
    normalized["album_title"] = clean_text(normalized.get("album_title", ""))
    bio = clean_text(normalized.get("persona_bio", ""))
    if not explicit_gender_supported:
        bio = neutralize_gendered_language(bio)
    normalized["persona_bio"] = bio

    tracklist_raw = normalized.get("tracklist", [])
    tracks = [clean_text(str(t)) for t in tracklist_raw if clean_text(str(t))]
    deduped = []
    seen = set()
    for t in tracks:
        key = normalize_for_similarity(t)
        if key and key not in seen:
            seen.add(key)
            deduped.append(t)
    normalized["tracklist"] = deduped[:10]
    return normalized


def build_last_resort_persona(
    model,
    tokenizer,
    author_label: str,
    themes: str,
    ...
    if obj is None:
        raise ValueError(
            f"{author_label}: could not parse fallback persona after retries ({retry_reason or 'unknown reason'})."
        )

    obj = normalize_persona_for_fallback(obj, explicit_gender_supported)
    ok, reason = validate_persona(obj)
    if not ok:
        raise ValueError(
            f"{author_label}: fallback persona was still invalid ({reason})."
        )

    has_issue, issue_reason = find_similarity_issue_relaxed(obj, prior_personas)
    if has_issue:
        print(
            f"⚠ {author_label}: accepting relaxed fallback persona after uniqueness warning: {issue_reason}",
            file=sys.stderr,
        )
    else:
        print(
            f"⚠ {author_label}: accepted fallback persona after strict retries were exhausted.",
            file=sys.stderr,
        )
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if has_issue:
            ...
            if best_candidate is None:
                best_candidate = dict(obj)
                best_candidate_reason = issue_reason
            continue

        return obj

    for _ in range(FINAL_RELAXED_ATTEMPTS):
        try:
            return build_last_resort_persona(
                model=model,
                tokenizer=tokenizer,
                author_label=author_label,
                themes=themes,
                ...
                retry_reason=last_failure or best_candidate_reason,
            )
        except ValueError as exc:
            last_failure = str(exc)

    if best_candidate is not None:
        relaxed_obj = normalize_persona_for_fallback(best_candidate, explicit_gender_supported)
        ok, reason = validate_persona(relaxed_obj)
        if ok:
            print(
                f"⚠ {author_label}: using best available persona after strict distinctiveness retries were exhausted ({best_candidate_reason or last_failure}).",
                file=sys.stderr,
            )
            return relaxed_obj
        last_failure = f"best candidate remained invalid after fallback normalization: {reason}"
