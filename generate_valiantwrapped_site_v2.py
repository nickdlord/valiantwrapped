...
def home_card_markup(author: AuthorRecord) -> str:
    ...
    return (
        ...
        f'{musician_portrait_markup(author) if author.musician_portrait_filename else cover_markup(author)}'
...
def collect_authors(
    ...
    portrait_map = {p.stem: p for p in musician_headshots_dir.glob("*.*") if p.suffix.lower() in {".png", ".jpg", ".jpeg", ".webp"}} if musician_headshots_dir.exists() else {}

    labels = sorted(set(metrics_map) | set(expertise_map) | set(persona_map) | set(rec_map) | set(cover_map) | set(portrait_map))
    ...
    for label in labels:
        ...
        authors.append(
            AuthorRecord(
                ...
                musician_portrait_filename=portrait_map.get(label).name if label in portrait_map else "",
