...
NEGATIVE_PROMPT = (
    "blurry, low resolution, distorted face, duplicate face, duplicate person, cloned person, mirrored face, "
    "extra limbs, extra fingers, bad anatomy, crossed eyes, asymmetrical eyes, warped teeth, "
    "two people, two faces, multiple people, crowd, background person, backup dancer, band members, audience, "
    "wide shot, full body, long shot, distant figure, tiny face, off-center subject, cropped face, cut-off head, "
    "profile view, extreme side view, face turned away, looking away, obscured face, hidden face, hair covering face, "
    "dull colors, muted colors, flat lighting, plain office portrait, academic headshot, corporate headshot, "
    "passport photo, linkedin photo, hospital portrait, lab coat portrait, neutral business attire, bland background, "
    "realistic office setting, plain wall, ordinary clothing, cartoon, illustration, anime, painting, watermark, "
    "text, logo, frame, collage"
)
...
def build_visual_prompt(persona: Persona, author_label: str) -> Tuple[str, str, Dict[str, str]]:
    ...
    prompt = (
        "single person only, one musician only, solo artist portrait, "
        "centered composition, symmetrical framing, subject filling the frame, "
        "tight head-and-shoulders portrait or upper-torso portrait, close-up editorial musician portrait, "
        "face clearly visible, eyes clearly visible, looking toward camera, "
        "designed for square album-style tile crop, square-cover-friendly composition, "
        "no additional people, no background figures, no duet, no bandmates, "
        "vivid imaginary musician transformation, "
        f"{genre}, {wardrobe}, {accents}, {lighting}, {background}, "
        f"{', '.join(mood_words)} energy, hyper-colorful fantasy editorial portrait, "
        ...
        "rich color palette, visually striking, highly stylized, "
        "professional studio portrait composition, cover-photo composition, clean focal subject"
    )
    ...
    prompt += (
        ", preserve one clear human subject only, "
        "keep the portrait tightly framed and centered for square website tiles"
    )
