...
def behind_the_scenes_line(author: AuthorRecord) -> str:
    if author.lifetime_pub_count > 0 or author.lifetime_citation_count > 0:
        return (
            "Not every standout year shows up on the charts. Some people are building the sound, "
            "keeping the production moving, and setting up the next big release behind the scenes."
        )
    return (
        "Still in the come-up era — building the sound, shaping the scene, and getting ready for a breakout run."
    )
