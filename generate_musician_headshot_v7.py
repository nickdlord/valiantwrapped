...
def trim_uniform_borders(
    ...
    """
    Trim near-uniform dark borders that often appear around generated portraits.
    This helps when the model bakes letterboxing or padding into the image itself.
    """
    img = img.convert("RGB")
    w, h = img.size
    if w <= 2 * sample_margin or h <= 2 * sample_margin:
        return img
    ...
    top = 0
    for y in range(h):
        if all(is_dark_uniform_pixel(x, y) for x in range(sample_margin, w - sample_margin)):
            top += 1
        else:
            break
    ...
    left = 0
    for x in range(w):
        if all(is_dark_uniform_pixel(x, y) for y in range(sample_margin, h - sample_margin)):
            left += 1
        else:
            break
...
def zoom_square_image(
    ...
    """
    Zoom into the image by cropping toward the center, then resize back out.
    zoom > 1.0 means a tighter crop.
    """
...
def finalize_tile_portrait(
    ...
    img = zoom_square_image(img, zoom=zoom, output_size=output_size)
    return img
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--tile-zoom", type=float, default=1.18, help="Post-generation center zoom used to make portraits fill square website tiles")
