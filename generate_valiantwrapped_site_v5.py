...
SECTION_KICKERS = {
    ...
    "persona": "Fictional Artist Reveal",
    "album": "Latest Album",
        ...
        for rec in author.recommendations:
            ...
            if rec.doi:
                secondary_actions.append(
                    f'<a class="secondary-link" href="https://doi.org/{html_escape(rec.doi)}" target="_blank" rel="noopener noreferrer">DOI</a>'
                )
            if rec.scopus_link:
                secondary_actions.append(
                    f'<a class="secondary-link" href="{html_escape(rec.scopus_link)}" target="_blank" rel="noopener noreferrer">Scopus</a>'
                )

            secondary_html = ""
            if secondary_actions:
                secondary_html = f'<div class="rec-secondary-actions">{" ".join(secondary_actions)}</div>'
...
def author_page_markup(author: AuthorRecord, project_title: str, tagline: str, closer: str) -> str:
    ...
    metrics_block = "\n".join([
        ...
        metric_chip("Favorite journal",
                    author.metrics.top_journal or "Still choosing a favorite"),
        metric_chip("Top paper citations", str(
            author.metrics.top_paper_citations)),
    ])
  ...
  <div class="app-shell">
    ...
    <main class="main-panel">
      <section class="hero author-hero">
        ...
        <div class="hero-art hero-portrait">{musician_portrait_markup(author, depth_prefix='../../')}</div>
      </section>

      <section id="publications" class="content-card">
        <div class="section-kicker">{html_escape(SECTION_KICKERS["publications"])}</div>
      ...
      <section id="persona" class="content-card">
        <div class="section-kicker">{html_escape(SECTION_KICKERS["persona"])}</div>
        <h2>Fictional Artist Reveal</h2>
...
.rec-secondary-actions { margin-top: 8px; display: flex; gap: 10px; flex-wrap: wrap; }
...
def list_normalized_file_map(path_glob: Sequence[Path]) -> Dict[str, Path]:
    out: Dict[str, Path] = {}
    for p in path_glob:
        out[canonical_author_label(p.stem)] = p
    return out
...
def collect_authors(
    ...
    labels = sorted(set(metrics_map) | set(expertise_map) | set(
        persona_map) | set(rec_map) | set(cover_map) | set(portrait_map))
    ...
    for label in labels:
        ...
        authors.append(
            AuthorRecord(
                ...
                persona=persona,
                cover_filename=cover_map.get(
                    label).name if label in cover_map else "",
                musician_portrait_filename=portrait_map.get(
                    label).name if label in portrait_map else "",
