...
def build_share_card(author: AuthorRecord, output_path: Path, cover_src_dir: Path, musician_headshot_src_dir: Path, project_title: str) -> None:
    ...
    draw.rounded_rectangle((24, 24, size - 24, size - 24), radius=48, fill=(15, 15, 15))
    ...
    draw.rounded_rectangle((54, 54, size - 54, size - 54), radius=42, fill=(11, 11, 11))
    ...
    draw.text((84, 110), 'Research, remixed.', font=font_eyebrow, fill=(242, 242, 242))

    share_label = '2025 Wrapped Share Card'
    share_bbox = draw.textbbox((0, 0), share_label, font=font_small)
    draw.text((size - 84 - (share_bbox[2] - share_bbox[0]), 76), share_label, font=font_small, fill=(188, 188, 188))
    ...
    draw.text((94, artist_box_top + 24), 'YOUR MUSICAL ALTER EGO', font=font_section, fill=(30, 215, 96))
    ...
    draw.text((406, stat_y + 5), 'Favorite journal', font=font_stat_label, fill=(173, 173, 173))
    journal_font = fit_font_for_multiline(draw, journal, 600, 20, min_size=15, bold=False, max_lines=2)
    draw_multiline_block(draw, journal, journal_font, (238, 238, 238), 406, stat_y + 25, 600, line_gap=3, max_lines=2)
    ...
    draw.text((78, heading_y), 'Latest Album:', font=label_font, fill=(30, 215, 96))
    album_title = author.persona.album_title or 'Untitled drop'
    album_title_font = fit_font_for_multiline(draw, album_title, 540, 36, min_size=20, bold=True, max_lines=2)
    draw_multiline_block(draw, album_title, album_title_font, (242, 242, 242), 78 + label_width + 16, heading_y - 4, 540, line_gap=2, max_lines=2)
    draw.text((724, heading_y), 'TRACKLIST', font=font_section, fill=(30, 215, 96))
    ...
    for i, track in enumerate(tracks, start=1):
        ...
        draw.rounded_rectangle((738, track_y, 1268, track_y + row_h - 6), radius=18, fill=(30, 30, 30))
        track_font = fit_font_for_multiline(draw, line, 488, 19, min_size=14, bold=False, max_lines=1)
