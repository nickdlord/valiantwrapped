...
STAGES = ["summary_txt", "expertise_txt", "persona_txt", "musician_headshot", "album_cover"]
...
def musician_headshot_path(musician_headshots_dir: Path, author: str) -> Path:
    for ext in (".png", ".jpg", ".jpeg", ".webp"):
        p = musician_headshots_dir / f"{author}{ext}"
        if p.exists():
            return p
    return musician_headshots_dir / f"{author}.png"
...
def run_per_author_pipeline(
    ...
    portrait_path = musician_headshot_path(musician_headshots_dir, author)
    ...
    if skip_existing and summary_path.exists():
        ...
        if rc == 0 and summary_path.exists():
            ...
            for s in ["expertise_txt", "persona_txt", "musician_headshot", "album_cover"]:
    ...
    if skip_existing and expertise_path.exists():
        ...
        if rc == 0 and expertise_path.exists():
            ...
            for s in ["persona_txt", "musician_headshot", "album_cover"]:
    ...
    if skip_existing and persona_path.exists():
        ...
        if rc == 0 and persona_path.exists():
            ...
            row.mark("musician_headshot", "blocked", "upstream failure at persona_txt")
            row.mark("album_cover", "blocked", "upstream failure at persona_txt")
            return

    # 4) musician headshot (more fictional prompt version)
    if skip_existing and portrait_path.exists():
        row.mark("musician_headshot", "skipped_existing", str(portrait_path))
    else:
        stdout_path, stderr_path = stage_log_paths(reports_dir, author, "musician_headshot")
        cmd = [
            python_exe,
            str(musician_headshot_script),
            "--project-root", str(project_root),
            "--input-file", str(persona_path),
            "--output-dir", str(musician_headshots_dir),
        ]
        if skip_existing:
            cmd.append("--skip-existing")
        rc = run_subprocess(cmd, stdout_path, stderr_path, cwd=project_root)
        portrait_path = musician_headshot_path(musician_headshots_dir, author)
        if rc == 0 and portrait_path.exists():
            row.mark("musician_headshot", "built", str(portrait_path))
        else:
            row.mark("musician_headshot", "failed", f"See {stdout_path.name} / {stderr_path.name}")
            row.mark("album_cover", "blocked", "upstream failure at musician_headshot")
            return
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--musician-headshots-dir", default="outputs/musician_headshots")
    ...
    ap.add_argument("--musician-headshot-script", default="generate_musician_headshot_iterable_dir.py")
    return ap
...
def main() -> None:
    ...
    musician_headshots_dir = (project_root / args.musician_headshots_dir).resolve()
    ...
    musician_headshot_script = (project_root / args.musician_headshot_script).resolve()
    ...
    ensure_dir(musician_headshots_dir)
    ...
    meta_lines = [
        ...
        f"musician_headshot_script: {musician_headshot_script}",
        "musician_headshot_prompt_mode: more_fictional_via_iterable_dir_script",
        f"outputs_ready_for_site_generation: author_summary_2025_present.csv, {expertise_txt_dir}, {persona_txt_dir}, {musician_headshots_dir}, {album_covers_dir}",
    ]
    ...
    for idx, author in enumerate(selected, start=1):
        ...
        run_per_author_pipeline(
            ...
            musician_headshot_script=musician_headshot_script,
            skip_existing=args.skip_existing,
        )
