...
def author_page_markup(author: AuthorRecord, project_title: str, tagline: str, closer: str) -> str:
    ...
    stat_items = [
        metric_chip("Total papers", str(total_papers if total_papers > 0 else "—")),
        metric_chip("Total citations", str(total_citations if total_citations > 0 else "—")),
    ]
    if clean_text(author.metrics.top_journal):
        stat_items.append(metric_chip("Top journal", author.metrics.top_journal))

    expertise_html = (
        f'<div class="summary-block"><div class="metric-label">Author expertise</div><div class="top-paper-title">{html_escape(author.expertise_summary)}</div></div>'
        if clean_text(author.expertise_summary)
        else '<div class="summary-block"><div class="metric-label">Author expertise</div><div class="top-paper-title">No expertise summary was available for this profile.</div></div>'
    )
    ...
    <main class="main-panel">
      <section class="hero author-hero">
        <div class="hero-copy hero-copy-only">
          ...
          <p class="section-copy hero-stats-copy">{html_escape(tagline)}</p>
          <div class="metrics-grid">{"".join(stat_items)}</div>
          {expertise_html}
        </div>
      </section>
