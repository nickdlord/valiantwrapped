...
def parse_author_identity_from_label(author_label: str) -> Dict[str, str]:
    ...
    if not parts:
        return {"scopus_id": "", "orcid": "", "id_type": "", "name_label": ""}
    ...
    if "ORCID" in upper_parts:
        ...
        return {"scopus_id": "", "orcid": orcid, "id_type": "orcid" if orcid else "", "name_label": "_".join(parts[:idx])}

    scopus_id = normalize_scopus_id(parts[-1]) if parts else ""
    if scopus_id and len(parts) >= 2:
        return {"scopus_id": scopus_id, "orcid": "", "id_type": "scopus", "name_label": "_".join(parts[:-1])}

    return {"scopus_id": "", "orcid": "", "id_type": "", "name_label": "_".join(parts)}
...
def load_lookup_row(lookup_csv: Path, scopus_id: str = "", orcid: str = "") -> Dict[str, str]:
    df = read_csv_flexible(lookup_csv).fillna("")
    required = {"record_id", "first_name", "last_name"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Lookup CSV missing required columns: {sorted(missing)}")

    if "scopus" in df.columns:
        df["scopus_norm"] = df["scopus"].map(normalize_scopus_id)
    else:
        df["scopus_norm"] = ""

    if "orcid" in df.columns:
        df["orcid_norm"] = df["orcid"].map(normalize_orcid)
    else:
        df["orcid_norm"] = ""

    match = pd.DataFrame()
    if scopus_id:
        match = df[df["scopus_norm"] == normalize_scopus_id(scopus_id)]
    if match.empty and orcid:
        match = df[df["orcid_norm"] == normalize_orcid(orcid)]

    if match.empty:
        ident = f"Scopus ID {scopus_id}" if scopus_id else f"ORCID {orcid}"
        raise FileNotFoundError(f"No row found in {lookup_csv} for {ident}")
...
def main() -> None:
    ...
    for idx, (persona_path, author_label) in enumerate(persona_items, start=1):
        ...
        try:
            ...
            scopus_id = identity["scopus_id"]
            orcid = identity["orcid"]
            lookup_row = load_lookup_row(lookup_csv, scopus_id=scopus_id, orcid=orcid)
