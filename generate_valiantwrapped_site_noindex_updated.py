...
2) Single-author TXT mode (new; for pipeline Option B)
   Example:
     python generate_valiantwrapped_site_noindex.py \
       ...
       --expertise-dir /tmp/.../author_expertise_txt \
       --album-covers-dir /tmp/.../album_covers \
       --scopus-db /tmp/.../scopusexportALL_02252026.csv \
       --docs-dir /tmp/.../docs \
...
def parse_author_scopus_id(author_label: str) -> str:
    parts = str(author_label).split("_")
...
def parse_expertise_summary_txt(expertise_path: Path) -> str:
    text = expertise_path.read_text(encoding="utf-8", errors="replace").strip()

    m = re.search(r"SUMMARY:\s*(.+)$", text, flags=re.IGNORECASE | re.DOTALL)
    if m:
        summary = m.group(1).strip()
        if summary:
            return summary

    # fallback: remove THEMES block labels if present and use remainder
    text = re.sub(r"^\s*THEMES:\s*", "", text, flags=re.IGNORECASE)
    text = re.sub(r"^\s*SUMMARY:\s*", "", text, flags=re.IGNORECASE | re.MULTILINE)
    return text.strip()
...
def load_recommender(scopus_db_path: Path, model_id: str, build_report: list):
    from sentence_transformers import SentenceTransformer
    import torch

    title_col = "Title"
    abstract_col = "Abstract"
    author_ids_col = "Author(s) ID"
    cited_by_col = "Cited by"
    year_col = "Year"
    source_title_col = "Source title"

    df = pd.read_csv(scopus_db_path, dtype=str, encoding="utf-8", low_memory=False)
    required = [title_col, abstract_col, author_ids_col]
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise ValueError(f"Scopus export is missing required columns: {missing}")

    df = df.fillna("")
    df["_title"] = df[title_col].astype(str).str.strip()
    df = df[df["_title"] != ""].copy()

    def build_candidate_text(row):
        parts = [row.get(title_col, "").strip()]
        abstract = row.get(abstract_col, "").strip()
        source_title = row.get(source_title_col, "").strip()
        if source_title:
    ...
    df["_candidate_text"] = df.apply(build_candidate_text, axis=1)
    df["_author_ids"] = df[author_ids_col].astype(str).apply(
        lambda s: {x.strip() for x in re.split(r"[;,]", s) if x.strip()}
    )
    df["_cited_by_num"] = pd.to_numeric(df.get(cited_by_col, ""), errors="coerce").fillna(0).astype(float)
    df["_year_num"] = pd.to_numeric(df.get(year_col, ""), errors="coerce").fillna(0).astype(float)

    if torch.cuda.is_available():
        device = "cuda"
    else:
        device = "cpu"
        build_report.append(("__recommender__", "cuda_not_detected", "Fell back to CPU for sentence embeddings."))

    model = SentenceTransformer(model_id, device=device)
    embeddings = model.encode(
        df["_candidate_text"].tolist(),
        batch_size=64,
...
def get_recommendations_for_author(
    recommender: dict,
    author_label: str,
    expertise_text: str,
    top_k: int,
    ...
    author_id = parse_author_scopus_id(author_label)
    if not expertise_text.strip():
        return []

    query_embedding = recommender["model"].encode(
        [expertise_text],
        convert_to_tensor=True,
        normalize_embeddings=True,
    ...
    scores = util.cos_sim(query_embedding, recommender["embeddings"])[0]
    score_values, score_indices = torch.topk(scores, k=min(len(scores), max(top_k * 8, 25)))

    recs = []
    seen_titles = set()

    df = recommender["df"]
    for score, idx in zip(score_values.tolist(), score_indices.tolist()):
        row = df.iloc[idx]
        if author_id and author_id in row["_author_ids"]:
            continue

        title = str(row["_title"]).strip()
        if not title:
            continue
        ...
        recs.append({
            "title": title,
            "google_url": google_search_url(title),
            "score": float(score),
...
def witty_uniqueness_message(first: str, last: str) -> str:
    display = f"{first} {last}".strip() or "This researcher"
    return (
        f"{html_lib.escape(display)} appears to be operating on a wavelength all their own. "
        "We searched the center-wide paper universe and came up mostly with distant cousins, "
        "not true twins. In other words: delightfully hard to categorize."
    )


def format_recommendations_html(recs, first: str, last: str) -> str:
    if not recs:
        return f"""
        <div class="reco-empty">
          <p>{witty_uniqueness_message(first, last)}</p>
        </div>
        """

    items = []
    for rec in recs:
        title = html_lib.escape(rec["title"])
        venue_bits = []
        if rec.get("source_title"):
            venue_bits.append(html_lib.escape(rec["source_title"]))
        if rec.get("year"):
            venue_bits.append(html_lib.escape(rec["year"]))
        ...
        items.append(
            ...
            <div class="reco-item">
              ...
              <div class="reco-meta">{venue}</div>
              <div class="reco-link-wrap">
                <a class="reco-link" href="{html_lib.escape(rec['google_url'])}" target="_blank" rel="noopener noreferrer">
    ...
    return f'<div class="reco-list">{"".join(items)}</div>'
...
def build_parser():
    ...
    ap.add_argument("--summary-dir", default="",
                    help="Folder containing per-author metrics summary TXT files (single-author mode)")
    ...
    # Related-paper recommender inputs
    ap.add_argument("--expertise-dir", default="author_expertise_txt",
                    help="Folder containing per-author expertise TXT files from author_expertise_llama31_2.py")
    ap.add_argument("--scopus-db", default="scopusexportALL_02252026.csv",
                    help="Mass Scopus export CSV used as the recommendation database")
    ap.add_argument("--rec-model-id", default="sentence-transformers/all-mpnet-base-v2",
                    help="SentenceTransformer model for related paper retrieval")
    ap.add_argument("--recommendation-count", type=int, default=5,
                    help="How many related papers to show per author")
    ap.add_argument("--min-similarity", type=float, default=0.33,
                    help="Minimum cosine similarity required to show a recommendation")
...
def main():
    ...
    expertise_dir = resolve_path(base_dir, args.expertise_dir)
    scopus_db_path = resolve_path(base_dir, args.scopus_db)
    ...
    if not expertise_dir.exists():
        raise FileNotFoundError(f"Expertise TXT directory not found: {expertise_dir}")
    if not scopus_db_path.exists():
        raise FileNotFoundError(f"Scopus database CSV not found: {scopus_db_path}")
    ...
    print(f"Loading paper recommender model: {args.rec_model_id}")
    recommender = load_recommender(scopus_db_path, args.rec_model_id, build_report)
    print(f"Loaded recommender on {recommender['device']} with {len(recommender['df'])} candidate papers.")
    ...
    def build_recommendation_block(author_label: str, first: str, last: str) -> str:
        expertise_path = expertise_dir / f"{author_label}.txt"
        if not expertise_path.exists():
            build_report.append((author_label, "missing_expertise_txt", str(expertise_path)))
            return format_recommendations_html([], first, last)

        expertise_text = parse_expertise_summary_txt(expertise_path)
        recs = get_recommendations_for_author(
        ...
        if not recs:
            build_report.append((author_label, "no_related_papers_found", f"threshold={args.min_similarity}"))
        return f"""
        <div class="reco-heading">Papers You Might Also Like</div>
        <p class="reco-sub">
          Based on your expertise summary, here are a few papers from across the center that seem closely aligned with your research interests.
        </p>
        {format_recommendations_html(recs, first, last)}
        """
    ...
    def generate_author_page(author_label: str):
        ...
        if p is None:
            ...
            recommendations_html = build_recommendation_block(author_label, first, last)
...
"""
    ...
    print(f"Related-paper database: {scopus_db_path}")
    print(f"Recommender model: {args.rec_model_id}")
