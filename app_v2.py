...
SITE_SCRIPT = BASE_DIR / "generate_valiantwrapped_site_v17.py"
...
def build_author_page_url(run: RunState) -> str:
    slug = re.sub(r"[^a-z0-9]+", "-", run.author_label.lower()).strip("-") or "author"
    return f"/runs/{run.run_id}/site/authors/{slug}/"
...
def execute_pipeline(run: RunState, first_name: str, last_name: str, scopus_id: str) -> None:
    ...
    try:
        ...
        set_step(run, "Building VALIANT Wrapped site", 93)
        run_subprocess(
            run,
            [
                python_exe,
        ...
        if not site_index.exists():
            raise FileNotFoundError(f"Expected site homepage was not created: {site_index}")
